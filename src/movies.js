...
function howManyMovies (movies) {
    const filteredArray = movies.filter( (movie) => ( movie.director === "Steven Spielberg" && movie.genre.includes("Drama")))
...
function orderAlphabetically (movies) {
    const alphabetList = [...movies];
    alphabetList.sort((a,b) => {
        if (a.title > b.title) {
            return 1;
          } else if (a.title < b.title) {
            return -1;
    ...
    const limited20List = alphabetList.filter( (movie,ind) => {
        return ind < 20
    })
    const titlesArray = limited20List.map( (title) => title.title)
    return titlesArray
}


console.log(orderAlphabetically(xxx))
...
function ratesAverage (movies) {
    ...
    if (!movies.length) {
        ...
        , 0)
        const average = rateSum / movies.length;
...
function convertHour (hourString) {
  let hour = hourString.split('h')
  return hour[0] * 60
}

function convertMin (minSring) {
  let min = minSring.split('m')
  return min[0] * 1
}

function convertDuration (duration) {
  let splitDurationArray = duration.split(' ');
  let calculatedDuration = splitDurationArray.reduce( (sum, splitDuration) => {
    if (splitDuration.includes('h')) {
      return sum + convertHour(splitDuration)
    }
    return sum + convertMin(splitDuration)
  }
  , 0)
...
function turnHoursToMinutes (movies) {
  let newDurationMoviesArray = movies.map( (movie) => {
    let newDurationMovie = {}
    newDurationMovie.title=movie.title
    newDurationMovie.year=movie.year
    newDurationMovie.genre=movie.genre
    newDurationMovie.director=movie.director
    newDurationMovie.rate=movie.rate
    newDurationMovie.duration=convertDuration(movie.duration)
    return newDurationMovie
  })
  return newDurationMoviesArray
}
